...
//    UIApplication.shared.registerUserNotificationSettings(mySettings)
  ...
  func userNotificationCenter(_ center: UNUserNotificationCenter,
    ...
    do{
      ...
      fatalError("error, could not fetch alarma from notifications: \(error)")
    }
    ...
    case "OK_ACTION":
      ...
      if (alertasMostradas < alertasTotales) {
        ...
        createLocalNotification(firedate: siguienteAlerta as NSDate, notification: response.notification, repeatNotification: false)
      }
      ...
      break
    case "DESPUES_ACTION":
      let despuesDe5 = Date().addingTimeInterval(60*5)
      
      createLocalNotification(firedate: despuesDe5 as NSDate, notification: response.notification, repeatNotification: false)
      break
  ...
  func createLocalNotification(firedate: NSDate, notification: UNNotification, repeatNotification: Bool)
  {
    ...
    let trigger = UNTimeIntervalNotificationTrigger(timeInterval: timeInterval, repeats: repeatNotification)
    
    print(timeInterval)
    
    let request = UNNotificationRequest(identifier: notification.request.content.userInfo["id"] as! String, content: localNotification, trigger: trigger)
